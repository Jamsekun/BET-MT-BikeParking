...
void manageBuzzer() {
    ...
    switch (buzzerState) {
        ...
        case STOLEN:
            ...
            if (elapsed >= buzzerCount * 2000 && elapsed < (buzzerCount * 2000 + 1000)) {
                tone(BUZZER_PIN, 1200, 1000); // 1200Hz for 1s
            } else if (elapsed >= (buzzerCount + 1) * 2000) {
                noTone(BUZZER_PIN);
                buzzerCount++;
...
void handleWiFiClient() {
    ...
    // Space 1, Actuator A
    if (request.indexOf("/space1/a/forward") != -1) {
        space1.stopActuators();
        delay(10);
        digitalWrite(SPACE1_A_LA_FORWARD, HIGH);
        digitalWrite(SPACE1_A_LA_REVERSE, LOW);
        Serial.println("WiFi: Space 1 A Forward");
        updateDisplay("Space 1 A Forward");
    } else if (request.indexOf("/space1/a/reverse") != -1) {
        space1.stopActuators();
        delay(10);
        digitalWrite(SPACE1_A_LA_REVERSE, HIGH);
        digitalWrite(SPACE1_A_LA_FORWARD, LOW);
        Serial.println("WiFi: Space 1 A Reverse");
        updateDisplay("Space 1 A Reverse");
    } else if (request.indexOf("/space1/a/off") != -1) {
        space1.stopActuators();
        Serial.println("WiFi: Space 1 A Off");
        updateDisplay("Space 1 A Off");
    }

    // Space 1, Actuator B
    if (request.indexOf("/space1/b/forward") != -1) {
        space1.stopActuators();
        delay(10);
        digitalWrite(SPACE1_B_LA_FORWARD, HIGH);
        digitalWrite(SPACE1_B_LA_REVERSE, LOW);
        Serial.println("WiFi: Space 1 B Forward");
        updateDisplay("Space 1 B Forward");
    } else if (request.indexOf("/space1/b/reverse") != -1) {
        space1.stopActuators();
        delay(10);
        digitalWrite(SPACE1_B_LA_REVERSE, HIGH);
        digitalWrite(SPACE1_B_LA_FORWARD, LOW);
        Serial.println("WiFi: Space 1 B Reverse");
        updateDisplay("Space 1 B Reverse");
    } else if (request.indexOf("/space1/b/off") != -1) {
        space1.stopActuators();
        Serial.println("WiFi: Space 1 B Off");
        updateDisplay("Space 1 B Off");
    }

    // Space 2, Actuator A
    if (request.indexOf("/space2/a/forward") != -1) {
        space2.stopActuators();
        delay(10);
        digitalWrite(SPACE2_A_LA_FORWARD, HIGH);
        digitalWrite(SPACE2_A_LA_REVERSE, LOW);
        Serial.println("WiFi: Space 2 A Forward");
        updateDisplay("Space 2 A Forward");
    } else if (request.indexOf("/space2/a/reverse") != -1) {
        space2.stopActuators();
        delay(10);
        digitalWrite(SPACE2_A_LA_REVERSE, HIGH);
        digitalWrite(SPACE2_A_LA_FORWARD, LOW);
        Serial.println("WiFi: Space 2 A Reverse");
        updateDisplay("Space 2 A Reverse");
    } else if (request.indexOf("/space2/a/off") != -1) {
        space2.stopActuators();
        Serial.println("WiFi: Space 2 A Off");
        updateDisplay("Space 2 A Off");
    }

    // Space 2, Actuator B
    if (request.indexOf("/space2/b/forward") != -1) {
        space2.stopActuators();
        delay(10);
        digitalWrite(SPACE2_B_LA_FORWARD, HIGH);
        digitalWrite(SPACE2_B_LA_REVERSE, LOW);
        Serial.println("WiFi: Space 2 B Forward");
        updateDisplay("Space 2 B Forward");
    } else if (request.indexOf("/space2/b/reverse") != -1) {
        space2.stopActuators();
        delay(10);
        digitalWrite(SPACE2_B_LA_REVERSE, HIGH);
        digitalWrite(SPACE2_B_LA_FORWARD, LOW);
        Serial.println("WiFi: Space 2 B Reverse");
        updateDisplay("Space 2 B Reverse");
    } else if (request.indexOf("/space2/b/off") != -1) {
        space2.stopActuators();
        Serial.println("WiFi: Space 2 B Off");
        updateDisplay("Space 2 B Off");
    }
    ...
    client.println("</head><body><div class='container'><h1 class='title'>Actuator Control</h1>");

    // Space 1 controls
    client.println("<div class='control-group'><h2 class='subtitle'>Space 1</h2>");
    client.println("<p>Actuator A:</p>");
    client.println("<a class='button is-primary' href='/space1/a/forward'>Forward</a>");
    client.println("<a class='button is-warning' href='/space1/a/reverse'>Reverse</a>");
    client.println("<a class='button is-danger' href='/space1/a/off'>Off</a>");
    client.println("<p>Actuator B:</p>");
    client.println("<a class='button is-primary' href='/space1/b/forward'>Forward</a>");
    client.println("<a class='button is-warning' href='/space1/b/reverse'>Reverse</a>");
    client.println("<a class='button is-danger' href='/space1/b/off'>Off</a></div>");

    // Space 2 controls
    client.println("<div class='control-group'><h2 class='subtitle'>Space 2</h2>");
    client.println("<p>Actuator A:</p>");
    client.println("<a class='button is-primary' href='/space2/a/forward'>Forward</a>");
    client.println("<a class='button is-warning' href='/space2/a/reverse'>Reverse</a>");
    client.println("<a class='button is-danger' href='/space2/a/off'>Off</a>");
    client.println("<p>Actuator B:</p>");
    client.println("<a class='button is-primary' href='/space2/b/forward'>Forward</a>");
    client.println("<a class='button is-warning' href='/space2/b/reverse'>Reverse</a>");
    client.println("<a class='button is-danger' href='/space2/b/off'>Off</a></div>");
